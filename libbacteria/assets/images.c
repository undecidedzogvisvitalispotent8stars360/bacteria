...
int gdInitColor( struct image * im , RGBa * color){
	return gdImageColorAllocateAlpha(im->im, color->r, color->g, color->b, color->a);
}

int gdSetColors(struct image *im, const int i, char * name, RGBa * color){
	if( i >  MAXCOLORS || i < 0) return -1;
...
void gdAddColors(struct image *im,char * name, RGBa * color){
	if( gdSetColors(im, cColors, name, color) == -1 ) return;
	cColors++;
}
...
#define MAXSIZEYX\
	if(x > im->width) return -1;\
	if(y > im->height) return -1;
...
struct image gdInitImage(ui width, ui height, 
		RGBa * bgcolor){
...
#define RANDCLR(alpha) gdImageColorAllocateAlpha(im->im,rand() % 255, rand() % 255, rand() %255, rand() % alpha)
...
void gdDrawRandomLines(struct image * im, ui count){
	  ui x, y,x1,y1;
	  while(count--){
	  	x = rand() % im->width; y = rand() % im->height;
	  	x1 = rand() % im->width; y1 = rand() % im->height;
...
#define RANDSTRCAPTCHA(a,b,c,file)\
	      gdGetRandStr( randStr, sizeof(randStr) );\
              gdImageStringTTF(im.im, NULL, black, file,\
      10, a, b, c, randStr);sprintf(rt.captcha,"%s%s",rt.captcha,randStr);
