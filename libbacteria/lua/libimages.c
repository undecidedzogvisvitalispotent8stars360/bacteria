...
INITLUAFUNC(gdInitImage){
	...
	printf("width:%d height:%d red:%d green:%d blue:%d alpha: %d\n", width,height, gd_red, gd_green, gd_blue, gd_alpha);
#endif
	RGBa color = {gd_red,gd_green,gd_blue};
 	size_t nbytes = sizeof(struct image) +
                  sizeof(gdImagePtr) + sizeof(ui)*(width-1) + sizeof(ui)*(height-1)+sizeof(char*); // TODO: to macros
...
#define GETIMAGE\
  ...
  if(in == NULL) luaL_error(L,"imagestruct is broken");

#define GETIMAGEDATA\
  struct ImageData *in = (struct ImageData *)lua_touserdata(L, 1);\
  if(in == NULL) luaL_error(L,"imagedata is broken");


INITLUAFUNC(gdFree){
  GETIMAGE;
  gdImageDestroy(in->im);
  if(in->textOnImage != NULL) free(in->textOnImage);
  in->width=0;
  in->height=0;
  lua_pushboolean(L,1);
  return 1;
...
INITLUAFUNC(gdInitImageFromFile){
	...
	size_t nbytes = sizeof(struct image) +
                  sizeof(gdImagePtr) + sizeof(ui)*(width-1) + sizeof(ui)*(height-1)+sizeof(char*);
  	struct image *ret = (struct image *)lua_newuserdata(L, nbytes);
	ret->im = d.im;
	ret->width=d.width;
	ret->height=d.height;
...
INITLUAFUNC(getImageData){
	...
	//printf("DataPtr: %p\n", data);
	size_t nbytes =
                  sizeof(unsigned char*) + sizeof(int)*(size-1);
